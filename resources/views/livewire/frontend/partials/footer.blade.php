<div class="mx-[19.71%] xxxl:mx-[14.71%] xxl:mx-[9.71%] xl:mx-[5.71%] md:mx-[12px]">
    ...
    <div class="flex flex-wrap justify-between gap-[35px] pt-[100px] pb-[58px] border-b border-white/20">
        ...
        <div class="max-w-[370px] xxs:max-w-full">
            {{-- <a href="{{ '/' }}" class="inline-block mb-[23px]" style="width: 4rem">
                <img src="{{ $company->small_logo ? asset($company->small_logo) : asset('/assetsBackend/media/avatars/blank.png') }}" alt="logo">
            </a> --}}
            <div class="logo flex items-center">
                <a href="{{ url('/') }}" class="flex items-center">
                    <img src="{{ $company->small_logo ? asset($company->small_logo) : asset('/assetsBackend/media/avatars/blank.png') }}" alt="logo" class="logo"
            ...
            <p class="text-[#D9D9D9] mb-[19px]">{{ $company->description }}</p>

            <ul class="space-y-[17px]">
                <li class="flex items-center gap-[8px]">
                    <span class="icon"><img src="{{ asset ('assetsFrontend/img/call-icon-yellow.svg')}}" alt="icon"></span>
                    <a href="tel:+6108-666-0112" class="hover:text-edyellow"> {{ $company->phone }} </a>
                </li>

                <li class="flex items-center gap-[8px]">
                    <span class="icon"><img src="{{ asset ('assetsFrontend/img/message-yellow.svg')}}" alt="icon"></span>
                    <a href="mailto:info@gmail.com" class="hover:text-edyellow">{{ $company->phone }}</a>
        ...
        <div>
            ...
            <div class="space-y-[18px]">
                <a href="#" class="flex items-center gap-[10px] opacity-80 hover:text-edyellow"><span
                        class="icon"><img src="{{ asset('assetsFrontend/img/double-arrow.svg')}}" alt="icon"></span> Edutics
                    About</a>
                <a href="#" class="flex items-center gap-[10px] opacity-80 hover:text-edyellow"><span
                        class="icon"><img src="{{ asset('assetsFrontend/img/double-arrow.svg')}}" alt="icon"></span> Contact
                    Us</a>
                <a href="#" class="flex items-center gap-[10px] opacity-80 hover:text-edyellow"><span
                        class="icon"><img src="{{ asset('assetsFrontend/img/double-arrow.svg')}}" alt="icon"></span> Blog &
                    News</a>
                <a href="#" class="flex items-center gap-[10px] opacity-80 hover:text-edyellow"><span
                        class="icon"><img src="{{ asset('assetsFrontend/img/double-arrow.svg')}}" alt="icon"></span> FAQ’S</a>
                <a href="#" class="flex items-center gap-[10px] opacity-80 hover:text-edyellow"><span
                        class="icon"><img src="{{ asset('assetsFrontend/img/double-arrow.svg')}}" alt="icon"></span> Login /
                    Register</a>
            </div>
        </div>
        ...
        <div>
            <h6
                class="font-semibold text-[18px] pb-[15px] mb-[30px] relative before:absolute before:bottom-0 before:left-0 before:h-[1.5px] before:w-[20px] before:bg-edyellow after:absolute after:bottom-0 after:left-[30px] after:h-[1.5px] after:w-[63px] after:bg-white">
                Navegación Principal</h6>

            <div class="space-y-[18px]">
                @forelse ($pages as $page)
                    {{-- <a href="{{ url('page', $page->slug) }}" target="_blank">{{ $page->title }}</a> --}}
                    <a href="{{ url('page', $page->slug) }}"
                        class="flex items-center gap-[10px] opacity-80 hover:text-edyellow"><span class="icon"><img
                                src="{{ asset('assetsFrontend/img/double-arrow.svg')}}" alt="icon"></span> {{ $page->title }}
                    </a>

                @empty
                    <a href="#" class="flex items-center gap-[10px] opacity-80 hover:text-edyellow"><span
                            class="icon"><img src="assetsFrontend/img/double-arrow.svg" alt="icon"></span> No hay
                        links para mostrar</a>
                @endforelse
        ...
        <div class="max-w-[300px]">
            <div>
                ...
                <div class="brutal-subscribe">
                    <div class="brutal-subscribe__container">
                        <div class="brutal-subscribe__header">
                            ...
                            <span class="brutal-subscribe__subtitle">QUIERES RECIBIR INFORMACIÓN</span>
                        </div>
                        <form class="brutal-subscribe__form">
                            <input type="email" class="brutal-subscribe__input" placeholder="tucorreo@ejemplo.com"
                                required="" />
                ...
                <div class="flex gap-[20px] mt-[30px]">
                    <span
                        class="pl-[30px] font-medium text-[#d9d9d9] relative before:absolute before:left-0 before:top-[50%] before:-translate-y-[50%] before:h-[1px] before:w-[20px] before:bg-[#d9d9d9]">Síguenos en</span>
                    <span class="inline-flex gap-[16px] text-[#d9d9d9]">
                        @foreach ($redes as $red)
                            <a href="{{ $red->url }}" class="hover:text-edyellow" target="_blank" rel="noopener noreferrer">
                                <i class="fa-brands {{ $red->icon_class }}"></i></a>
